...
## Layout
- `src/` — TypeScript source: `index.ts` (CLI), `models.ts` (`loadModels()`), `tokenizer.ts` (`estimateTokens()` — labeled token **range** + `isApproximate`; heuristic), `estimate.ts` (`estimateOutputTokens`/`estimateCost`/`formatEstimate` — labeled USD cost **ranges**, pure + unit-tested), `claude.ts` (`resolveClaudePath`/`buildClaudeArgs`/`runClaude` — the safe hand-off). `run` prints the plain-text pre-run report (default model `claude-sonnet-4-6`; `--model` to switch), then **execs the real `claude --model <id>`** (stdio inherited, exit code passed through); `--dry-run` analyzes only. Loader/unknown-model errors surface as a clean one-line `promptmeter: <msg>` + exit 1.
