...
- **Hand-off safety:** the spawn uses `shell:false` + an argv array (no command injection); the task/model are never put in a shell. `PROMPTMETER_CLAUDE_BIN` overrides which `claude` binary runs (test seam / escape hatch). PromptMeter does **not** shim or rename `claude` — it stays reachable (full reversible interception is Phase 1.5).
